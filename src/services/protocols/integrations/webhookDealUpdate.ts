export interface IWebhookDealUpdateDataInfo {
  email_messages_count: number;
  cc_email: string;
  channel?: string;
  products_count: number;
  acv_currency?: string;
  next_activity_date: string;
  acv?: string;
  next_activity_type: string;
  local_close_date?: string;
  next_activity_duration?: string;
  id: number;
  person_id: number;
  creator_user_id: number;
  expected_close_date: string;
  owner_name: string;
  arr_currency?: string;
  participants_count: number;
  stage_id: number;
  probability?: string;
  undone_activities_count: number;
  active: boolean;
  local_lost_date?: string;
  person_name: string;
  last_activity_date?: string;
  close_time?: string;
  org_hidden: boolean;
  next_activity_id: number;
  weighted_value_currency: string;
  local_won_date?: string;
  stage_order_nr: number;
  next_activity_subject: string;
  rotten_time?: string;
  user_id: number;
  visible_to: string;
  org_id: number;
  notes_count: number;
  next_activity_time?: string;
  channel_id?: string;
  formatted_value: string;
  status: "won" | "open";
  formatted_weighted_value: string;
  mrr_currency?: string;
  first_won_time?: string;
  origin?: string;
  last_outgoing_mail_time?: string;
  origin_id?: string;
  title: string;
  last_activity_id?: string;
  update_time: string;
  activities_count: number;
  pipeline_id: number;
  lost_time?: string;
  currency: string;
  weighted_value: number;
  org_name: string;
  value: number;
  person_hidden: boolean;
  next_activity_note?: string;
  arr?: string;
  files_count: number;
  last_incoming_mail_time?: string;
  label?: string;
  mrr?: string;
  lost_reason?: string;
  deleted: boolean;
  won_time?: string;
  followers_count: number;
  stage_change_time: string;
  add_time: string;
  done_activities_count: number;
}

export interface IWebhookDealUpdateMetaDataInfo {
  action: "add" | "merge" | "delete" | "change";
  change_source: string;
  company_id: number;
  host: string;
  id: string;
  is_bulk_update: boolean;
  matches_filters: { current: [] };
  object: string;
  permitted_user_ids: number[];
  pipedrive_service_name: boolean;
  timestamp: number;
  timestamp_micro: number;
  prepublish_timestamp: number;
  trans_pending: boolean;
  user_id: number;
  v: number;
  webhook_id: string;
}

export interface IWebhookPreviousDataInfo {
  email_messages_count: number;
  cc_email: string;
  channel?: string;
  products_count: number;
  acv_currency?: string;
  next_activity_date: string;
  acv?: string;
  next_activity_type: string;
  local_close_date?: string;
  next_activity_duration?: string;
  id: number;
  person_id: number;
  creator_user_id: number;
  expected_close_date: string;
  owner_name: string;
  arr_currency?: string;
  participants_count: number;
  stage_id: number;
  probability?: string;
  undone_activities_count: number;
  active: true;
  local_lost_date?: string;
  person_name: string;
  last_activity_date?: string;
  close_time?: string;
  org_hidden: boolean;
  next_activity_id: number;
  weighted_value_currency: string;
  local_won_date?: string;
  stage_order_nr: number;
  next_activity_subject: string;
  rotten_time?: string;
  user_id: number;
  visible_to: string;
  org_id: number;
  notes_count: number;
  next_activity_time?: string;
  channel_id?: string;
  formatted_value: string;
  status: string;
  formatted_weighted_value: string;
  mrr_currency?: string;
  first_won_time?: string;
  origin?: string;
  last_outgoing_mail_time?: string;
  origin_id?: string;
  title: string;
  last_activity_id?: string;
  update_time: string;
  activities_count: number;
  pipeline_id: number;
  lost_time?: string;
  currency: string;
  weighted_value: number;
  org_name: string;
  value: number;
  person_hidden: boolean;
  next_activity_note?: string;
  arr?: string;
  files_count: number;
  last_incoming_mail_time?: string;
  label?: string;
  mrr?: string;
  lost_reason?: string;
  deleted: boolean;
  won_time?: string;
  followers_count: number;
  stage_change_time?: string;
  add_time: string;
  done_activities_count: number;
}

export interface IWebhookDealUpdateData {
  meta: IWebhookDealUpdateMetaDataInfo;
  data: IWebhookDealUpdateDataInfo;
  previous: IWebhookPreviousDataInfo;
  retry: number;
  event: string;
}
